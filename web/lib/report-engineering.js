...
export function buildEngineeringReportArtifacts(analysis, {
  ...
  const reportParts = [
    ...
    markdownTable(["Evidence class", "Status"], [
      ...
      ["Research Deployment Sensitivity", deployCurvatureResult ? "run" : "not run"],
    ]),
    "",
    executionPlacementMarkdown(executionPlacement),
    "",
    ...
    markdownTable(["Metric", "Value"], [
      ...
      ["Quantized tensor coverage", formatPercent(quant.quantized_tensor_percent)],
      ...
      [onnx ? "Reduced-precision estimate" : ["not_quantized_float", "float16_weight_storage"].includes(quant.classification) ? "Reduced-precision opportunity" : "Quantized-path planning estimate", onnx
        ...
        : !analysis.quantization_status?.label || String(quant.compute_mac_assessment || "").startsWith("not_") || !Number.isFinite(analysis.estimated_int8_speedup)
          ? "NOT_ASSESSABLE; a bound quantization contract and complete MAC denominator are required for a compute-kernel ceiling"
    ...
    markdownTable(["Module", "Status", "Key point"], [
      ["Experimental artifact composites", deepBomResult?.status || (deepBomResult ? "complete" : "not run"), deepBomResult ? (deepBomResult.score_assessment?.status === "ASSESSED" ? `artifact=${score100(deepBomResult.basin_proxy_score)} topology=${score100(deepBomResult.topology_stress_score)}; unvalidated fixed weighting` : `NOT_ASSESSABLE: ${deepBomResult.score_assessment?.reason || "required score inputs unavailable"}`) : "No experimental artifact-composite result."],
      ...
      ["Research Deployment Sensitivity", deployCurvatureResult?.status || "not run", deployCurvatureResult?.basin ? `experimental composite=${Number(deployCurvatureResult.basin.score || 0).toFixed(1)} / 100; no validated decision threshold` : "No deployment-sensitivity result."],
    ]),
    "",
