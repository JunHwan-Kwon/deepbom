...
export function buildMlBomCompatibilityProjection(analysis, {
  ...
  const componentProperties = compact([
    ...
    property("mlbom:model:totalMacs", analysis?.total_macs_decimal ?? finite(analysis?.total_macs)),
    ...
    property("mlbom:model:quantizedComputeMacs", quant.quantized_compute_macs_decimal ?? finite(quant.quantized_compute_macs)),
    property("mlbom:model:computeMacs", quant.compute_macs_decimal ?? finite(quant.compute_macs)),
