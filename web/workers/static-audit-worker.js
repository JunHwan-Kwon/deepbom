...
function ensureWasm(id) {
  if (!wasmReady) {
    ...
    tfliteModulePromise ||= import("../../pkg/tflite_wasm_audit.js");
    wasmReady = tfliteModulePromise.then(async (module) => {
...
self.addEventListener("message", async ({ data }) => {
  ...
  try {
    ...
    const tflite = String(operation).startsWith("tflite_") ? await ensureWasm(id) : null;
    let result;
    if (operation === STATIC_AUDIT_OPERATION.TFLITE_ANALYZE) {
      ...
      result = tflite.compute_deployment_frontier(requestModelBytes, requestFilename, payload.targetIdsJson);
      ...
      result = tflite.compute_delegation_repair(requestModelBytes, requestFilename, payload.targetId);
      ...
      result = tflite.compute_deployment_delta(
      ...
      result = tflite.compute_input_influence(requestModelBytes, requestFilename, payload.opIndex, payload.targetId);
    } else if (operation === STATIC_AUDIT_OPERATION.TFLITE_OUTPUT_INFLUENCE) {
      status(id, "Tracing selected-operator output influence");
      result = tflite.compute_output_influence(requestModelBytes, requestFilename, payload.opIndex, payload.targetId);
      ...
      result = tflite.explore_tflite_redesign_pareto(requestModelBytes, requestFilename, payload.targetId, payload.request);
    } else if (operation === STATIC_AUDIT_OPERATION.TFLITE_MODEL_TOMOGRAPHY) {
      status(id, "Computing model tomography");
      result = tflite.compute_model_tomography(requestModelBytes, requestFilename, payload.targetId);
    } else if (operation === STATIC_AUDIT_OPERATION.TFLITE_LAYER_LANDSCAPE) {
      status(id, "Computing the selected layer landscape");
      result = tflite.layer_landscape_grid(
        requestModelBytes,
        payload.opIndex,
        payload.seed1,
        payload.seed2,
      ...
      result = tflite.landscape_tomography(requestModelBytes, payload.numProjections, payload.gridSize, payload.radius);
    } else if (operation === STATIC_AUDIT_OPERATION.TFLITE_SYNTHETIC_LANDSCAPE) {
      status(id, "Computing the synthetic float landscape");
      result = tflite.synthetic_landscape_grid(requestModelBytes, payload.seed1, payload.seed2, payload.gridSize, payload.radius);
      ...
      result = tflite.landscape_directions(requestModelBytes, payload.seed1, payload.seed2);
    } else if (operation === STATIC_AUDIT_OPERATION.TFLITE_KERNEL_HAAR) {
      status(id, "Computing static kernel Haar evidence");
      result = tflite.compute_kernel_haar_decomposition(requestModelBytes, requestFilename, payload.targetId);
    } else if (operation === STATIC_AUDIT_OPERATION.TFLITE_ACTIVATION_HAAR) {
      status(id, "Computing synthetic activation Haar evidence");
      result = tflite.compute_activation_haar(requestModelBytes, requestFilename, payload.targetId);
      ...
      const { analyzeOnnxModel } = await import("../onnx.js");
      ...
      const [{ analyzeExecuTorchModel }, { resolveExecuTorchSelectedBuildAttestation }] = await Promise.all([
        import("../executorch.js"),
        import("../lib/executorch-build-binding.js"),
      ]);
