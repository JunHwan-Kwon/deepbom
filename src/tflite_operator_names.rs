...
const PLACEHOLDER_FOR_GREATER_OP_CODES: i32 = 127;

pub(super) fn operator_code_name(fb: &Fb, table: usize) -> Result<String, String> {
    let deprecated =
        fb.checked_i8_field(table, 0, 0, "OperatorCode.deprecated_builtin_code")? as i32;
    let custom = fb
        .checked_string_field(table, 1, "OperatorCode.custom_code")?
        .unwrap_or_default();
    let builtin_code = fb.checked_i32_field(table, 3, 0, "OperatorCode.builtin_code")?;
    let code = if builtin_code < PLACEHOLDER_FOR_GREATER_OP_CODES {
        deprecated
    } else {
        builtin_code
    };
    Ok(builtin_operator_name(code, &custom))
}

fn builtin_operator_name(code: i32, custom: &str) -> String {
    if !custom.is_empty() {
...
#[cfg(test)]
mod tests {
    use super::builtin_operator_name;

    #[test]
    fn resolves_builtin_custom_and_unknown_operator_names() {
        assert_eq!(builtin_operator_name(3, ""), "CONV_2D");
        assert_eq!(builtin_operator_name(114, ""), "QUANTIZE");
        assert_eq!(builtin_operator_name(209, ""), "STABLEHLO_CASE");
        assert_eq!(builtin_operator_name(999, ""), "OP_999");
        assert_eq!(builtin_operator_name(32, "vendor.custom"), "vendor.custom");
    }
}
