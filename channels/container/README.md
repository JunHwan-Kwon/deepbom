...
```bash
docker build --build-arg DEEPBOM_VERSION=1.96.15 -t deepbom:1.103.0 -f channels/container/Dockerfile .
docker run --rm -v "$PWD:/workspace:ro" deepbom:1.103.0 audit model.onnx --summary
```
