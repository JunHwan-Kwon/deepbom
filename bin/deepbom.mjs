...
const VERSION = typeof __DEEPBOM_RELEASE_VERSION__ === "string" ? __DEEPBOM_RELEASE_VERSION__ : "1.104.0";
...
async function main(argv) {
  ...
  if (parsed.weightAnalysis || parsed.activationEvidence) {
    ...
    if (parsed.weightAnalysis) {
      ...
      const { buildWeightIr } = await import("../web/lib/weight-ir.js");
      await input.file.assertUnchanged();
      analysis.weight_ir = await buildWeightIr(artifactIrContext.model_ir, analysis, input.file);
      await input.file.assertUnchanged();
    }
