...
assert.match(zeroStructureReview.title, /^1 of 1 assessed serialized tensor payloads are exactly all-zero$/);
assert.match(zeroStructureReview.summary, /1\/1 fully decoded tensor payload/);
assert.equal(zeroStructureReview.affected_tensor_count, 1);
assert.equal(zeroStructureReview.assessed_tensor_count, 1);
assert.deepEqual(zeroStructureReview.affected_tensors, ["weight"]);
assert.equal(zeroStructureReview.affected_tensors_truncated, false);
