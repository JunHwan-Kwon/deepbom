...
for (const [key, field, expected] of [
  ...
  ["cyclonedx20Preview", "requireModelBytes", true],
  ["cyclonedx20Preview", "ensureHash", true],
  ["cyclonedx20Preview", "type", "application/vnd.cyclonedx+json; version=2.0-draft"],
  ["cyclonedx20Preview", "raw", true],
...
expectEqual(engineeringReportArtifacts.metricCoverage.schema, "deepbom.metric_coverage_manifest.v1.53", "Engineering Report should emit field-level metric coverage schema v1.53 with ExecuTorch ET12/FT01 serialized-contract ownership, effective ONNX opset-import resolution, runtime internal-shape cost closure, runtime copy-node movement boundaries, core-allocation roofline evidence, artifact byte ownership, representative-dataset validation, source-pinned TFLite control-flow contracts, ONNX recursive-scope intrinsic-cost conservation, sparse-storage ownership, LLM MoE/SSM/runtime contracts, TensorRT preflight ownership, and the existing static/runtime decision bindings.");
...
expectEqual(decisionCoverage.schema, "deepbom.decision_coverage.v1.10", "Decision coverage should expose a versioned machine-readable schema with ExecuTorch serialized-contract assignment, ONNX runtime internal-shape closure, runtime copy-node movement boundaries, core-allocation roofline, complete TFLite GPU/NNAPI deployment-candidate assignment, TensorRT preflight ownership, and SafeTensors packed-weight quantization ownership while preserving decision-domain conservation.");
