...
// Common IR type/member/metric/provenance contracts and source rederivation (about 30 KiB).
const handwrittenRuntimeBudgetKiB = Number(process.env.HANDWRITTEN_RUNTIME_SOURCE_BUDGET_KIB || process.env.SOURCE_BUDGET_KIB || 12800);
const generatedRuntimeDataBudgetKiB = Number(process.env.GENERATED_RUNTIME_DATA_BUDGET_KIB || 1728);
// Common IR review, typed schema definitions and hardening implementation record.
const docsSourceBudgetKiB = Number(process.env.DOCS_SOURCE_BUDGET_KIB || 800);
// Common IR regression fixtures and canonical-hash mutation checks.
const verificationSourceBudgetKiB = Number(process.env.VERIFICATION_SOURCE_BUDGET_KIB || 3824);
// Source-pin/support gate manifest adds about 4 KiB of explicit review metadata.
const devToolingSourceBudgetKiB = Number(process.env.DEV_TOOLING_SOURCE_BUDGET_KIB || 1700);
