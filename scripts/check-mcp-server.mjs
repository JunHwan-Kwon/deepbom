...
async function checkRealServerContract() {
  ...
  try {
    ...
    session.request(29, "tools/call", {
      name: "deepbom_audit",
      arguments: { path: onnxPath, section: "model_summary" },
    });
    const modelSummarySelection = (await session.response(29)).result.structuredContent;
    assert.equal(modelSummarySelection.schema, "deepbom.analysis_selection.v1");
    assert.equal(modelSummarySelection.sections.model_summary.schema, "deepbom.model_summary.v1");
    assert.equal(modelSummarySelection.sections.model_summary.source_contract.artifact_sha256, envelope.identity.sha256,
