...
const sourceFiles = ["web/lib/report-export-contracts.js", "web/lib/public-cyclonedx-export.js"];
const names = new Set();
for (const sourceFile of sourceFiles) {
  const source = await readFile(sourceFile, "utf8");
  for (const match of source.matchAll(/\["(deepbom:[^"]+)"/g)) names.add(match[1]);
}
...
const properties = [...names].sort().map((name) => ({
  ...
  reconciliation: stable.has(name) ? "artifact_observable_allowlist" : "not_compared_by_default",
}));
