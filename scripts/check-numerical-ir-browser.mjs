...
try{
  ...
  const saved=JSON.parse(await readFile(await(await save("save")).path(),"utf8"));assert.deepEqual(saved.weight_ir,expected);assert.equal(saved.bundle.source.artifact_sha256,sha);
  ...
  await page.locator('[type="file"]').setInputFiles({name:"capture.json",mimeType:"application/json",buffer:Buffer.from(JSON.stringify(capture))});
  ...
  await page.locator('[type="file"]').setInputFiles({name:"invalid.json",mimeType:"application/json",buffer:Buffer.from('{"schema":"deepbom.activation_capture.v1"}')});
