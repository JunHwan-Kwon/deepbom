...
`deepbom_cyclonedx_evidence.cdx.json` is the primary schema-valid AI/ML-BOM. The other files are hash-bound companion evidence for tensor contracts, observed formulation, runtime requirements, and provenance gaps. `deepbom_cyclonedx_2_0_draft_compatibility.json` records the commit-pinned integration status of the interdependent CycloneDX 2.0 drafts. It is not a CycloneDX 2.0 BOM or a conformance claim, and the unresolved draft profile refuses 2.0 export. Missing source declarations are reported as missing or not assessable; they are not fabricated to make the example appear complete.
