...
`deepbom_cyclonedx_evidence.cdx.json` is the primary schema-valid AI/ML-BOM. The other files are hash-bound companion evidence for tensor contracts, observed formulation, runtime requirements, and provenance gaps. `deepbom_cyclonedx_2_0_parameter_contract.preview.cdx.json` is a separate commit-pinned draft projection for parameter-bound AI/ML properties; it is not a CycloneDX 2.0 conformance claim. Missing source declarations are reported as missing or not assessable; they are not fabricated to make the example appear complete.
